...
int parse_return_nonce(BT_AS_INFO dev, uint8_t *rx_data, uint16_t rx_len)
{
	...
	*/
	...
	for(j = 0; j < sizeof(fpga_ret_q->nonce)/sizeof(*fpga_ret_nonce_q); j++) //保存nonce 数??
	{
		uint8_t data;
		uint16_t work_id;
				...
				*/
				...
				else
				{
					...
					gNonce_Err++;
						...
						for(chain_loop = 0; chain_loop < 16; chain_loop++)
						{
							if(chain_hw[chain_loop] != 0)
								{
									printk(KERN_ERR "Error WorkID = %x",work_id);
									printk(KERN_ERR "chain%d - HW%d \n",chain_loop + 1,chain_hw[chain_loop]);
								}
						}
				}
			}
